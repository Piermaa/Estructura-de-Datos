...
[Serializable]
    ...
    public class GrafoLA : IGrafoTDA
    {
        ...
        */
        ...
        public int PesoCamino(List<NodoGrafo> nodos)
        {
            int pesoTotal=0;
            for (int i = 0; i < nodos.Count-1; i++)
            {
                if (ExisteArista(nodos[i].valorNodo,nodos[i+1].valorNodo))
                {
                    pesoTotal += PesoArista(nodos[i].valorNodo, nodos[i+1].valorNodo);
                }
                else
                {
                    Debug.LogWarning($"No existe arista entre: {nodos[i].valorNodo} y {nodos[i+1].valorNodo}");
                    return pesoTotal;
                }
            }
